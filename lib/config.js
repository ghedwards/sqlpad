...
var config = rc('sqlpad', {
    ...
    engine: process.env.SQLPAD_ENGINE || "ejs",
    dbPath: path.join(userHome, "sqlpad/db"),
    baseUrl: process.env.SQLPAD_BASE_URL || "",
    passphrase: process.env.SQLPAD_PASSPHRASE || "At least the sensitive bits won't be plain text?"
});

// Support running server on base path
config.baseUrl = config['base-url'] || config.baseUrl
...
if (config.h || config.help) {
    var helpText = "\n" 
                 ...
                 + "  --base-url [path]           Base url to run on (optional. Default is empty string)\n"
